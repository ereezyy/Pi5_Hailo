...
export function DetectionHeatmap() {
  ...
  // ⚡ Bolt: Pre-compute class totals to prevent O(C*N) recalculations
  // in the render loop. Previously, the render loop iterated over classes
  // and called `.filter()` and `.reduce()` on the entire `heatmapData` array
  // for each class, which scales poorly.
